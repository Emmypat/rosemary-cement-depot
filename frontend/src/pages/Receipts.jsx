...
const MOCK_RECEIPTS = [
  { id: 1, sale_id: 1, customer_name: 'John Mensah', customer_email: 'john@example.com', total: 3200, emailed_to: 'john@example.com', sent_at: new Date().toISOString(), created_at: new Date().toISOString() },
  { id: 2, sale_id: 2, customer_name: 'Abena Owusu', customer_email: 'abena@example.com', total: 5600, emailed_to: 'abena@example.com', sent_at: new Date().toISOString(), created_at: new Date().toISOString() },
  { id: 3, sale_id: 4, customer_name: 'Walk-in', customer_email: null, total: 680, emailed_to: null, sent_at: null, created_at: new Date().toISOString() },
]
              ...
              {filtered.map(receipt => (
                <tr key={receipt.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-slate-500">
                    <span className="text-slate-400">#</span>{receipt.id}
                    <span className="text-xs text-slate-400 ml-1">(Sale #{receipt.sale_id})</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => downloadMutation.mutate(receipt.id)}
